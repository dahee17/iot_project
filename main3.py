...
def main_loop():
    global feeding_status, remaining_text, beeped, countdown_start, completed_start
    ...
    while True:
        ...
        # 먼저 Completed 상태 타임아웃 처리
        if feeding_status == "Completed" and completed_start is not None:
            if time.time() - completed_start >= COMPLETED_TIMEOUT:
                feeding_status = "Idle"
                remaining_text = ""
                completed_start = None
                # 리셋 beep 플래그도 초기화
        ...
        # Countdown 상태 타임아웃 처리 (거리 감지로 진입하는 카운트다운)
        if feeding_status == "Countdown" and countdown_start is not None:
            if time.time() - countdown_start >= COUNTDOWN_TIMEOUT:
                feeding_status = "Idle"
                remaining_text = ""
                countdown_start = None
                beeped = False
